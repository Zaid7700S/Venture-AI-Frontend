...
export default function PlanRenderer({ markdown, onReset }) {
  ...
  return (
    <div className="animate-fade-in w-full">
      ...
      <div className="flex flex-col-reverse sm:flex-row sm:flex-wrap items-stretch sm:items-center justify-between gap-3 sm:gap-4 mb-6 sm:mb-8 pb-6 border-b border-gray-100 no-print">
        <button 
          onClick={onReset} 
          className="text-primary font-semibold hover:text-primary-dark flex items-center justify-center sm:justify-start gap-2 transition-colors group py-2"
        >
        ...
        <button 
          ...
          className="flex items-center justify-center gap-2 bg-gray-900 text-white px-5 py-2.5 rounded-xl hover:bg-gray-800 transition-all shadow-sm active:scale-[0.98] font-medium text-sm"
        >
      ...
      <div id="plan-document" className="bg-white p-4 sm:p-8 md:p-12 rounded-2xl shadow-sm border border-gray-100">
        <ReactMarkdown 
          remarkPlugins={[remarkGfm]}
          components={{
            h1: ({node, ...props}) => <h1 className="text-2xl sm:text-3xl font-extrabold text-primary border-b-2 border-primary/20 pb-3 mb-6 mt-8 first:mt-0" {...props} />,
            h2: ({node, ...props}) => <h2 className="text-xl sm:text-2xl font-bold text-gray-900 mt-8 mb-4 flex items-center gap-2" {...props} />,
            h3: ({node, ...props}) => <h3 className="text-base sm:text-lg font-bold text-gray-800 mt-6 mb-3 border-l-4 border-primary pl-3" {...props} />,
